...
using static Natrix.ParkingUtils.PathInfo;
using Cell = Natrix.ParkingUtils.PathInfo.Cell;
using Natrix.Utils;
...
namespace Natrix
{
    public class ParkingUtils
    {
        ...
        public static void PathFinder4(Parking Parking, ref int startCellfindingattempt)
        {
            var mtx = Parking.PlanMatrix;
            ...
            double CountAdjacency(int row, int col)
            {
                ...
                for (int k = 0; k < 4; k++)
                {
                    ...
                    else if (mtx[rr, cc] == 3) num += 0.25;
                    else if (mtx[rr, cc] == 2) num += 0.25;
                }
                return num;
            }
            ...
            const double straightPathBonusFactor = 2;
            // =================================================================================
        ...
        new GH_Path(pathindex, n + 1, m)
            ...
            if (adjacencynum.Max() > 2)
            {
                ...
                // ==== TUNABLE PARAMETER =====================================================
                // selectionSharpness controls how strongly a direction's probability of being
                // chosen depends on its weighted score. Each score is raised to this power
                // before being turned into a probability:
                //   1.0   -> probability directly proportional to the score (classic roulette
                //            wheel / fitness-proportionate selection)
                //   > 1.0 -> higher-scoring directions become disproportionately more likely
                //            to be picked (closer to the old "always take the max" behavior)
                //   < 1.0 (down towards 0) -> probabilities flatten towards uniform, i.e. more
                //            randomness/exploration regardless of score
                // Adjust this later based on how the generated layouts turn out.
                const double selectionSharpness = 10;
                // =============================================================================

                // Turn the 4 direction scores into a probability distribution and draw one
                // direction from it, instead of always taking the (possibly tied) maximum.
                var selectionWeights = new double[4];
                double totalSelectionWeight = 0;
                for (int dir = 0; dir < 4; dir++)
                {
                    selectionWeights[dir] = weightedAdjacency[dir] > 0 ? Math.Pow(weightedAdjacency[dir], selectionSharpness) : 0;
                    totalSelectionWeight += selectionWeights[dir];
                }

                int nextcell = 0;
                if (totalSelectionWeight <= 0)
                {
                    // Safety net: should not normally happen here since adjacencynum.Max() > 2
                    // guarantees at least one usable direction, but falls back to a uniform
                    // pick among the raw-score maxima if it ever does.
                    double rawMax = adjacencynum.Max();
                    var fallbackCandidates = Enumerable.Range(0, adjacencynum.Length).Where(i => adjacencynum[i] == rawMax).ToList();
                    nextcell = fallbackCandidates[new Random().Next(fallbackCandidates.Count)];
                }
                else
                {
                    double roll = new Random().NextDouble() * totalSelectionWeight;
                    double cumulative = 0;
                    for (int dir = 0; dir < 4; dir++)
                    {
                        cumulative += selectionWeights[dir];
                        if (roll <= cumulative)
                        {
                            nextcell = dir;
                            break;
