...
namespace ParkingDemo.Utils
{
    internal class ParkingPreview
    {
        public static void PrintResult(string DocumentName, int Index, Rectangle3d PlanArea, bool resetPlan)
        {
            ...
            {
                viewRectangle = doc.Views.AddPageView($"Parking Plan_{new Random().Next(100)}", PlanArea.Width, PlanArea.Height);
            }
            ...
            viewRectangle.AddDetailView("Plan", corner0, corner1, DefinedViewportProjection.Top);
            ...
            var bitmapnew = view_capture.CaptureToBitmap(viewRectangle);
               ...
               Size size = new Size(width*15, height*15);
               var image2 =
                   viewRectangle.GetPreviewImage(size, true);
               image2.SetResolution(300, 300);
            image2.MakeTransparent();
            // image.Save(fileName);
            if (null != image2)
            {
                string filename2 = $"C:\\Parking\\Result\\01\\Parking Plan_{Index.ToString()}.png"; // Change filename and format here
