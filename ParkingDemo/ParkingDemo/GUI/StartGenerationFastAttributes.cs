...
namespace ParkingDemo
{
    ...
    {
        private readonly RectangleF[] _buttons = new RectangleF[11];
        ...
        protected override void Layout()
        {
            ...
            Bounds = RectangleF.Union(body, new RectangleF(x, y, width, 146));
        }
        ...
        protected override void Render(GH_Canvas canvas, Graphics graphics, GH_CanvasChannel channel)
        {
            ...
            string[] labels = { Component.AddRamp ? "Ramp: On" : "Ramp: Off", "North", "West", "East", "South",
                "10 s", "20 s", "1 min", "Until Stop", "START", "STOP" };
            ...
            for (int i = 0; i < _buttons.Length; i++)
            {
                bool active = i == 0 ? Component.AddRamp : i < 5 ? (int)Component.EntranceSide == i - 1 :
                    i < 9 ? Component.DurationSeconds == _durations[i - 5] : i == 9 ? Component.Running : !Component.Running;
                using (var capsule = GH_Capsule.CreateTextCapsule(_buttons[i], _buttons[i],
                    active ? GH_Palette.Black : GH_Palette.Normal, labels[i]))
                    capsule.Render(graphics, Selected, Owner.Locked || (Component.Running && i < 9), false);
            }
        }
        ...
        public override GH_ObjectResponse RespondToMouseDown(GH_Canvas sender, GH_CanvasMouseEvent e)
        {
            ...
            for (int i = 0; i < _buttons.Length; i++)
            {
                ...
                else if (!Component.Running)
                {
                    ...
                    else Component.SetDuration(_durations[i - 5]);
                }
                sender.Refresh();
